...
- **P4** 迭代→技能晋升 ✅ 全链路打通：采收 → 候选(admin tab) → LLM 起草/管理员编辑确认 → 写全局库 →
  ...
  门槛 = 管理员确认(硬门) + 跨租户>5 自动推荐(可越级)。**全局库已有第一条真技能**（2026-06-28 实跑 promote：
  JP/currency 候选→LLM 起草「日元货币代码标准化」→写全局库→attach→渲进 prompt，全链贯通）。
  ...
  **promote 写全局库已实跑**（2026-06-28，全局库 0→1，全链端到端验证通过）。
...
- **ADR-002 typed-edit 优化器** ✅ **默认开启**（2026-07-07，`SKILL_TYPED_EDITS=True` +
  ...
  **两次真实灰度通过**（ADR-002 §9.3 达成，两 API 两 provider 五指标全过）：
  - **L1**（my-invoice-abf4f0 · gemini · 3 轮，期间 Gemini 两次断连仍安全）：
    accepted 32/rejected 5、单调 0.546→0.591、演化 26 次、**正文冻结 PASS**、
    verifier unavailable=0。
  - **L1.3**（my-invoice-11ec4a-c1 · qwen · 3 轮）：accepted 94/rejected 61、
    单调 0.55→0.56、**META hint 注入 27 次**（"replace 拒绝率 67% → 优先 append"）、
    修复后**正文冻结 PASS**。首跑（修复前）暴露了 provider 依赖的冻结软保证 bug。
  回退：`SKILL_TYPED_EDITS=false` 字节级等价回旧整段重写路径。
  详见 [ADR-002](./ADR-002-typed-edits-meta-skill.md)。
