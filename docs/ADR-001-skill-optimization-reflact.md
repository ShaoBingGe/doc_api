...
### P4 — 迭代→技能晋升（固化进全局库）　🔶 进行中（步骤①采收已落地）
**晋升门槛（2026-06-26 拍板）：跨租户覆盖 > 5  AND  管理员确认。** golden_set 不回归作为给
管理员的**参考信息**展示，不作硬卡（替代原「三与门」中的频率项）。
- [x] **步骤①采收（只读）**：`service/skill_promotion.py` 扫同国家各 API 的 `skill_feedback`，按
  `(国家,字段)` 聚成候选 + 跨租户计数 + `qualified=tenant>5` 标记。纯函数单测 7 绿；生产实跑挖出
  11 候选（4 JP / 7 MY），因生产仅 1 租户故 0 够格（符合预期）。**绝不写技能。**
- [ ] 步骤②给管理员展示候选 + golden_set 参考分（只读 endpoint + 技能库面板）。
- [ ] 步骤③管理员确认 → 建全局 `OcrSkill`（NULL）；步骤④ attach 到 module → composer 渲染（已上线）。
