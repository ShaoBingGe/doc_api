...
> **进度：四阶段全部完成** ✅
> P0 `353b78d` · P1 `c5b8cae` · P2 `7e798e0` · P3 `444022a`
>
> 用户三论断 + 两个隐藏缺陷全部关死：
> - #1/#3：客户可从 UI 创建带列明细表，response_schema 对行结构有完整约束；
> - #2：多数组组装本就支持，现在客户也能新增第二个数组；
> - 列改名半失效 → P2 全链级联（schema+prompt+标注整列）；
> - 数组列无跨样本反思语料 → P3 解禁 + 按列聚合。
>
> 延后项（次要 polish）：P1 空表预览；D6 的 pad []。
> P2 列编辑与 P3 行编辑只对顶层数组开放（`$[*].arr[*]`，嵌套路径无此场景）。
...
- [x] 客户可从 UI 新增带列数组字段，response_schema 对行结构有完整 items 约束
- [x] 多个数组字段（模板 + 客户新增）可同时输出（composer 实测多数组组）
- [x] 列加/删/改名全链生效（schema + prompt + 标注级联），列改名不再半失效
